...
impl NginxGenerator {
    pub fn generate_and_install(
        ...
        let tmp_conf_path = PathBuf::from(format!("{TMP_BASE_PATH}/{site_name}.conf"));

        if let Some(parent_dir) = tmp_conf_path.parent() {
            fs::create_dir_all(parent_dir)?;
        }

        let conf_text = Self::nginx_template(&site_name, port);
        fs::write(&tmp_conf_path, conf_text)?;

        let target_conf_path = format!("{NGINX_SITES_AVAILABLE}/{site_name}.conf");
        let tmp_conf_string = tmp_conf_path
            .to_str()
            .ok_or_else(|| anyhow!("invalid nginx temp path"))?;

        privilege_wrapper.run("/usr/bin/mv", &[tmp_conf_string, &target_conf_path])?;
